...
const DriverEdit = () => {
  ...
  const { drivers, updateDriver } = useAdmin();

  const existing = drivers.find((d) => d.id == id) || {};

  const [form, setForm]       = useState({ name: existing.name || "", email: existing.email || "", phone: existing.phone || "", licenseNumber: existing.licenseNumber || "", status: existing.status || "Available", avatar: existing.avatar || "" });
  ...
  const submit = (e) => {
    e.preventDefault();
    setTouched({ name: true, email: true, phone: true, licenseNumber: true, status: true, avatar: true });
  ...
  return (
    <div className="py-4 max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-white">Edit Driver</h2>
        ...
        <form onSubmit={submit} className="space-y-5" noValidate>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="License Number *" error={err("licenseNumber")}>
              <input value={form.licenseNumber} onChange={(e) => set("licenseNumber", e.target.value)} onBlur={() => blur("licenseNumber")} className={cls} />
            </Field>
          ...
          <Field label="Avatar URL" error={err("avatar")}>
            <input value={form.avatar} onChange={(e) => set("avatar", e.target.value)} onBlur={() => blur("avatar")} className={cls} />
